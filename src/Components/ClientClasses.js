        ...
        <div className="fullSpace">
          {classes
            .filter((item) => {
              ...
              const classMatchesSearchTerm = () => {
                let fields = [];

                fields.push(item.type);
                fields.push(item.location);
                fields.push(item.intensity);
                fields.push(item.instructor_name);
              ...
              if (searchTerm === "") {
                return true;
              } else if (classMatchesSearchTerm()) {
                return true;
              } else {
                return false;
              }
                ...
                <div className="client-class">
                  <div className="class-content-container">
                    <div className="class-header">
                      <h3>{item.type}</h3>
                      <h4>
                        <text className="bold">Instructor:</text> {item.instructor_name}
                      </h4>
                      <p>
                        <text className="bold">When:</text> {item.date} -- {item.time}
                      </p>
                      <p>
                        <text className="bold">Level:</text> {item.intensity}
                      </p>
                      <p>
                        <text className="bold">Location:</text> {item.location}
                      </p>
                    </div>
